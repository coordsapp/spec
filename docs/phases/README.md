# Phases

These are the engineering phases for the Coords Cloud / hosted SaaS build (spec + core + cloud). They are unrelated to Landblock's own adoption stages (see `cloud/docs/landblock-overview.md`) — Landblock deliberately uses the word "Stage," not "Phase," so the two numbering schemes never collide in conversation.

Phase documentation index:

- [Phase 1 - Open Protocol Foundation](./phase-01.md) - `Completed`
- [Phase 2 - Resolver MVP](./phase-02.md) - `Completed`
- [Phase 3 - Alias Claiming and Verified Ownership](./phase-03.md) - `Completed`
- [Phase 4 - Enterprise Features and Scale](./phase-04.md) - `Partially Completed` (billing/domains/SLA shipped; SAML/SCIM and resolver-usage analytics never built — see the doc)
- [Phase 5 - Warehouse and Dock Management](./phase-05.md) - `Completed`
- [Phase 6 - Landing Experience and Developer Onboarding](./phase-06.md) - `Completed`
- [Phase 7 - Interactive Coordinate Converter](./phase-07.md) - `Completed`
- [Phase 8 - Open Source Map Platform](./phase-08.md) - `Completed`
- [Phase 9 - Active Logistics Coordination](./phase-09.md) - `In Progress`
- [Phase 10 - Coords XREF System (CXS): Design and Discussion](./phase-10.md) - `Planning`

See `docs/open-questions.md` for tracked issues and gaps found across these phases.
