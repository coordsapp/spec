...
1. open `/map` for workspace mode or `/explore` for classic mode
2. search handles or click map coordinates
3. load viewport overlays for warehouses, docks, and handles
4. share exact map context via URL parameters

## Scope
- Map web routes:
1. `GET /map` (workspace UI)
2. `GET /explore` (classic explorer)
- New viewport API:
...
1. streets
2. terrain
3. satellite with roads and labels overlays
...
2. `/v1/map/resolve`
- Workspace map (`/map`) delivered with:
...
5. shareable URL state (`center`, `zoom`, `q`, `layers`, `basemap`)
6. compact basemap toggle with satellite roads/labels overlays
7. route planning panel integrated with Phase 9 planning API
...
1. `cloud/handlers/web/web_map_v2.go` (`/map`)
2. `cloud/handlers/web/web_map.go` (`/explore`)
